...
class NPM(LM):
    ...
    def tokenize(self, texts, skip_special_tokens=False, padding=True):
        ...
        for i, input_ids in enumerate(all_input_ids):
            ...
            _all_attention_mask.append(
                [1 for _ in range(n_valid)] + [0 for _ in range(n_masks)]
            )
    ...
    def encode(self, texts, skip_special_tokens=False, gt_input_ids=None):
        ...
        all_input_ids, all_attention_mask = self.tokenize(
            texts, skip_special_tokens=skip_special_tokens
        )
        ...
        for i, (text, input_ids, logits, hidden_states) in enumerate(
            ...
            if self.mask_id in input_ids:
                ...
                _hidden_states = [
                    h for _id, h in zip(input_ids, hidden_states) if _id not in [0, 2]
                ]
    ...
    def get_probabilty(self, topic, question):
        ...
        if cache_key not in self.cache_dict:
            ...
            question_input_ids = self.tokenize(
                ["Fact: " + question], skip_special_tokens=False, padding=False
            )[0]
            ...
            for i, input_id in enumerate(question_input_ids):
                ...
                batch.append(
                    self.decode(
                        question_input_ids[:i]
                        + [self.mask_id]
                        + question_input_ids[i + 1 :]
                    )
                )
                gt_input_ids.append(input_id)
            for (prob, vector), gt_input_id in zip(
                self.encode(batch, gt_input_ids=gt_input_ids), gt_input_ids
            ):
                triples.append((prob, vector, gt_input_id))

            stacked_question_vectors = np.stack([v for _, v, _ in triples], 0)
            all_scores = np.exp(
                np.inner(stacked_question_vectors, stacked_passage_vectors)
                / np.sqrt(stacked_passage_vectors.shape[-1])
            )
