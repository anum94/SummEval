...
class AtomicFactGenerator(object):
    def __init__(self, openai_key, demon_dir, gpt3_cache_file=None):
        ...
        self.openai_lm = OpenAIModel(
            "gpt-4o-mini", cache_file=gpt3_cache_file, openai_key=openai_key
        )
...
def get_atomic_facts_for_paragraph(paragraph: str):
    working_dir = os.path.dirname(os.path.realpath(__file__))
    factscore_cache_dir = os.path.join(working_dir, ".cache/factscore")
    factscore_demos_dir = os.path.join(factscore_cache_dir, "demos")
    ...
    generator = AtomicFactGenerator(openai_key=api_key,
                                    demon_dir=factscore_demos_dir,
                                    gpt3_cache_file=os.path.join(factscore_cache_dir, "GPT4o-mini.pkl"))
    atomic_facts, paragraph_breaks = generator.run(generation=paragraph)
    generator.save_cache()

    transformed_atomic_facts = [
        {
            'sentence': sentence,
            'facts': facts
        }
    ...
    return {
        'atomic_facts': transformed_atomic_facts,
...
if __name__ == "__main__":
    # example execution
    response = get_atomic_facts_for_paragraph(
        paragraph="Thierry Henry (born 17 August 1977) is a French professional football coach, pundit, "
                  "and former player. He is considered one of the greatest strikers of all time, and one the greatest "
                  "players of the Premier League history. He has been named Arsenal F.C's greatest ever player.")
    print(response)
